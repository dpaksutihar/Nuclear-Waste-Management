export const fetchFarmer = async (curr_address, farmerContract, id) => {
  const response = await farmerContract.methods
    .getFarmer(id)
    .call({ from: curr_address });
  return {
    ...response.farmer,
    formattedAddress:
      id.substring(0, 6) + "..." + id.substring(id.length - 4, id.length),
...
export const fetchStakeholder = async (
  ...
  const response = await stakeholderContract.methods
    .get(id)
    .call({ from: curr_address });
  return {
    ...response,
...
export const formattedAddress = (address) => {
  if (!address) {
    return "N/A"; // Handle undefined or null addresses
  }
